...
type tGameController struct {}
var GameController tGameController


func (_ tGameController) Games(
		) string {
	args := make(map[string]string)
	
	return revel.MainRouter.Reverse("GameController.Games", args).Url
}

func (_ tGameController) CreateGame(
		name string,
		) string {
	args := make(map[string]string)
	
	revel.Unbind(args, "name", name)
	return revel.MainRouter.Reverse("GameController.CreateGame", args).Url
}
